...
class Network(object):
    ...
    def feedforward(self, x):
        for layer_idx in range(self.num_layers-1):
            ...
            x = np.vstack(a) + self.biases[layer_idx]
            x = sigmoid(x+self.biases[layer_idx])
        ...
        for layer_idx in range(self.num_layers-1):
            ...
            for i in range(self.cir_matrix_num[layer_idx][0]):
                for j in range(self.cir_matrix_num[layer_idx][1]):
                    ...
                    a[i] += getFFTout(self.cir_weights[layer_idx][i, j].T, x_input[j])
            z = np.vstack(a) + self.biases[layer_idx]
            zs.append(z)
            # print(z)
...
def getFFTout(x, y):
    ...
    res = np.real(np.fft.ifft(np.fft.fft(x) * np.fft.fft(y))).reshape(-1, 1)
